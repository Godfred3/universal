...
export default function LoginScreen() {
  ...
  const { signInWithEmail, isAuthorized } = useAuth();
  ...
  useEffect(() => {
    if (navigateToApp && isAuthorized) {
      router.replace('/(app)');
    }
  }, [isAuthorized, navigateToApp, router]);
  ...
  const handleLogin = async () => {
    ...
    setSubmitting(true);
    const { authorized, error } = await signInWithEmail(email, password);
    setSubmitting(false);
    if (error) {
